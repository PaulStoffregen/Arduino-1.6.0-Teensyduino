...
//    }
  ...
  protected void populatePortMenu() {
    ...
    for (BoardPort port : ports) {
      ...
      boolean checked = address.equals(selectedPort);
      if (checked) anychecked = true;
      JCheckBoxMenuItem item = new JCheckBoxMenuItem(label, checked);
      item.addActionListener(new SerialMenuListener(address));
      serialMenu.add(item);
    }
    if (selectedPort == null || anychecked == false) {
      serialMenu.setText("Port");
    } else {
      serialMenu.setText("Port: \"" + selectedPort + "\"");
    }
